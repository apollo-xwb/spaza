...
export default function PanelSheet({ open, tab, onClose, children }: PanelSheetProps) {
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-x-0 bottom-0 z-50 flex flex-col glass-panel-strong rounded-t-3xl border-t border-border md:hidden grunge-pattern"
        style={{ height: "calc(85vh - env(safe-area-inset-bottom))" }}
      >
        <div className="flex items-center justify-between px-5 pt-4 pb-2 shrink-0">
          <div>
            <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-border" />
            <h2 className="text-base font-bold text-primary">{titles[tab]}</h2>
          </div>
          <button onClick={onClose} className="text-muted hover:text-primary text-2xl">×</button>
        </div>
